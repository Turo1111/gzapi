...
import { getBrand, getBrands, insertBrand, updateBrand } from '../services/brand'
...
const getItem = async ({ params }: RequestExt, res: Response): Promise<void> => {
  try {
    const { id } = params
    const response = await getBrand(new Types.ObjectId(id))
    res.send(response)
  } catch (e) {
...
const uptdateItem = async ({ params, body }: Request, res: Response): Promise<void> => {
  try {
    const { id } = params
    const response = await updateBrand(new Types.ObjectId(id), body)
    emitSocket('brand', {
      action: 'update',
      data: body
    })
...
const postItem = async ({ body }: Request, res: Response): Promise<void> => {
  try {
    const response = await insertBrand(body)
    emitSocket('brand', {
      action: 'create',
      data: response
    })
